...
# ── bslib Dark Theme ──────────────────────────────────────────────────────────
# Fonts loaded via Google Fonts CDN in ui.R head tags.
shaw_theme <- bs_theme(
  version   = 5,
  bg        = "#080b10",
  fg        = "#d4cfc8",
...
# ── FRED Fetch (called once on login if key is present) ───────────────────────
...
# ── Mock Macro Data (fallback — approximate historical values) ─────────────────
# ⚠ Data Notes:
#   housing_starts : Approximate quarterly averages of FRED HOUST (k units SAAR).
#                    Source: FRED HOUST series historical data, manually rounded.
...
#                    The 2021–22 spike was real and well-documented.
#   cpi            : Approximate quarterly averages of FRED CPIAUCSL (Index 1982-84=100).
#   construction   : Approximate quarterly averages of FRED TTLCONS ($B, rescaled).
#   freight_ppi    : Approximate quarterly averages of FRED PCU484121484121 (Index 2012=100).
...
#                    (b) MHK (Mohawk) public revenue as industry benchmark
#                    (c) Floor Covering Weekly industry shipments trends
#                    (d) Housing starts correlation model (r ≈ 0.87 to HOUST)
#                    Do NOT present as Shaw-reported revenue.
MOCK_MACRO <- tibble(
...
# Source: Floor Covering Weekly annual market share estimates; approximate.
# Hard-surface share shift is well-documented in trade press.
FLOOR_SHARE <- tibble(
  year     = 2019:2025,
...
# ── Competitor Data ───────────────────────────────────────────────────────────
# Source: Public SEC 10-K / annual reports for public companies.
# Shaw and Engineered Floors are modeled estimates (private companies).
# Mohawk gross margin: FY2023 10-K reported gross profit / net sales.
# Interface gross margin: FY2023 10-K.
# Armstrong gross margin: FY2023 10-K (Armstrong World Industries; flooring).
COMPETITORS <- tibble(
  company   = c("Shaw (Est.)",         "Mohawk Ind. (MHK)",   "Interface (TILE)",
                "Armstrong (AWI)",     "Tarkett SA"),
  revenue_b = c(6.2,                   10.9,                  1.37,
                1.27,                  2.85),
  gm_pct    = c(NA_real_,              29.6,                  38.0,
                35.1,                  22.1),
  source    = c("Analyst est. — private", "MHK FY2023 10-K",  "TILE FY2023 10-K",
                "AWI FY2023 10-K",     "Tarkett 2023 Ann. Rpt"),
  color     = c(PAL$accent, PAL$blue, PAL$green, PAL$purple, PAL$muted)
)

# ── Porter Radar ──────────────────────────────────────────────────────────────
# Scores: analyst qualitative judgement (0–100 = competitive pressure intensity).
# Benchmark: hypothetical average for mature U.S. industrial manufacturing sector.
PORTER_RADAR <- tibble(
  force     = c("Industry\nRivalry","New\nEntrants","Supplier\nPower",
                "Buyer\nPower","Substitutes"),
...
PORTER_FORCES <- list(
  rivalry = list(
    ...
    evidence = c(
      ...
      "Mohawk gross margin compressed from ~34% (FY2018) to ~29.6% (FY2023) per 10-K filings — structural industry-wide pricing pressure",
      "Engineered Floors, Mannington, and Shaw's own Anderson Tuftex compete in the mid-market, fragmenting margin pools",
      "SPC/LVT intensifies rivalry: Mohawk (RevWood Plus), Shaw (COREtec, new SPC Feb 2025), and Asian imports all competing in the fastest-growing segment",
  ...
  entrants = list(
    ...
    evidence = c(
      ...
      "U.S. Census Bureau trade data shows import penetration in floor coverings (HS 5703, 3918) growing steadily — the effective 'new entrant' is Asian LVT",
      "Section 301 tariffs on Chinese LVT imposed in 2018 partially slowed China, but Vietnamese production has grown to partially offset",
      "No major greenfield domestic flooring plant by a new entrant announced in 7+ years (trade press review: Floor Covering Weekly, Floor Covering News)"
    ),
  ...
  suppliers = list(
    ...
    evidence = c(
      "FRED WPU0672 (PPI: Plastics Products) rose approximately 36% from Q1 2020 to peak in Q2 2022 before partially normalizing — a real, verifiable data point",
      "Invista (nylon 6,6), Ascend Performance Materials (nylon 6,6), and RadiciGroup control majority of supply for carpet-grade nylon fiber",
      "Mohawk FY2023 10-K explicitly cites 'raw material cost volatility including petroleum-derived materials' as a primary business risk",
      ...
      "Shaw's new SPC product line introduces additional input dependencies: LVT core SPC materials and wear-layer resins predominantly sourced from Asia"
    ),
  ...
  buyers = list(
    ...
    evidence = c(
      ...
      "Estimated 10,000+ independent flooring dealers in the U.S. (Floor Covering Weekly data) — highly fragmented; individual retail dealer buyer power is LOW",
  ...
  substitutes = list(
    ...
    evidence = c(
      "Floor Covering Weekly market estimates: carpet ~38% of U.S. flooring dollars (2019) to ~28% (2024) — a 10-percentage-point decline in 5 years",
      "LVT/SPC share: est. 16% (2019) to ~30% (2024) per trade press — this growth is largely sourced from carpet's share loss",
      "American Pet Products Association: 66% of U.S. households owned a pet in their 2023–24 survey — strongly correlated with hard-surface preference due to cleanability",
      "Shaw's COREtec LVT brand is one of its fastest-growing lines — creating deliberate internal cannibalization of its legacy carpet business",
      "Mohawk FY2023 investor day: management explicitly acknowledged 'accelerating shift from soft to hard surface' as a multi-year structural trend"
    ),
...
SWOT_DATA <- list(
  S = list(label = "Strengths", color = PAL$green, items = list(
    list(title = "#1 U.S. Flooring Manufacturer",
         evidence = "Estimated ~26–27% of U.S. flooring manufacturing revenue per Floor Covering Weekly rankings. Shaw is privately held; exact revenue not disclosed.",
    ...
    list(title = "Berkshire Hathaway Capital Access",
         evidence = "Full subsidiary since 2002. BRK maintains exceptional financial strength; effectively unlimited long-term patient capital for M&A and capex cycles without public market pressure.",
    ...
    list(title = "Vertical Integration Advantage",
         evidence = "Shaw controls fiber extrusion through manufacturing through distribution — enabling cost and quality control advantages vs. assembler-model competitors. Mohawk has pursued a similar but less complete vertical model.",
         indicator = "Shaw COGS as % of estimated revenue vs. Mohawk gross margin (reported in MHK 10-K)",
         risk = 18, tier = "HIGH"),
    list(title = "Multi-Brand Portfolio",
         evidence = "9 brands spanning residential (Shaw Floors, Anderson Tuftex, COREtec), commercial (Patcraft, Philadelphia, Shaw Contract), and specialty (Shawgrass, Southwest Greens, Shaw Sports Turf)",
  ...
  W = list(label = "Weaknesses", color = PAL$red, items = list(
    ...
    list(title = "Carpet Mix Transition Cost",
         evidence = "Shaw CEO Tim Baucom acknowledged in trade press interviews the 'difficult and costly' transition from solid-color nylon carpet to patterned polyester as consumer preferences shifted post-2020. Patterned construction requires different manufacturing line configurations and higher SKU complexity.",
    ...
    list(title = "No Public Financial Disclosure",
         evidence = "As a private Berkshire subsidiary, Shaw does not publish standalone financials. This limits external benchmarking, complicates competitive analysis, and constrains investor-facing strategy communication.",
         indicator = "N/A — structural to private ownership model",
         risk = 28, tier = "LOW"),
    list(title = "U.S. Revenue Concentration",
         evidence = "Shaw maintains offices in 13 countries but estimated >80% of revenue is U.S.-sourced based on Berkshire segment commentary. High concentration to U.S. macro cycle with limited international diversification.",
  ...
  O = list(label = "Opportunities", color = PAL$blue, items = list(
    list(title = "SPC/LVT Market Growth",
         evidence = "Floor Covering Weekly trade data shows LVT/SPC at est. 30%+ of U.S. flooring dollars (2024). Multiple market research firms project continued double-digit CAGR for global SPC. Shaw's COREtec is established; the Feb 2025 SPC line extends further.",
         indicator = "Census HS 3918 import volumes; LVT/SPC category volume in Floor Covering Weekly",
         risk = 18, tier = "HIGH"),
    ...
    list(title = "AI & Manufacturing Optimization",
         ...
         indicator = "Capital expenditure disclosures from MHK on automation; industry adoption benchmarks",
         risk = 28, tier = "MED"),
    list(title = "Tariff-Driven Reshoring Tailwind",
         ...
         indicator = "USTR tariff action announcements; Census HS 3918 monthly import volumes by country",
         risk = 35, tier = "MED"),
  ...
  T = list(label = "Threats", color = PAL$amber, items = list(
    list(title = "Sustained Elevated Mortgage Rates",
         evidence = "FRED MORTGAGE30US averaged ~6.8–7.1% throughout 2024. At 6.79% vs. 2.77% (Q4 2020 low), the rate differential has a meaningful suppressive effect on existing home sales and residential remodeling demand — the flooring industry's largest end market.",
    ...
    list(title = "Asian LVT/SPC Import Competition",
         evidence = "Vietnamese and Chinese manufacturers produce LVT at estimated 20–35% cost discount to U.S. domestic production (Floor Covering Weekly trade estimates). Census HS 3918 import data confirms growing import share despite Section 301 tariffs on China, as production shifts to Vietnam.",
         indicator = "U.S. Census Bureau monthly trade: HS codes 3918 + 5703; USTR tariff review schedule",
         risk = 65, tier = "HIGH"),
...
# ── Scenarios ─────────────────────────────────────────────────────────────────
# ⚠ All revenue/EBITDA figures are analyst estimates — Shaw does not publish financials.
